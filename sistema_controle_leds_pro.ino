/*
  ...
  - Seleção de Fonte: MANUAL, DMX, ou APP/RF (BLE)
...
// Fontes de Controle
enum ControlSource { SOURCE_MANUAL, SOURCE_DMX, SOURCE_APP };
const char* source_names[] = {"MANUAL", "DMX", "APP/RF"};
...
class MyCallbacks: public BLECharacteristicCallbacks {
    void onWrite(BLECharacteristic *pCharacteristic) {
      if (current_source != SOURCE_APP) return; // Só aceita se a fonte for APP
...
void handleDMX() {
  if (current_source != SOURCE_DMX) return; // Só aceita se a fonte for DMX
...
void loop() {
  ...
  if(is_menu_mode) {
    ...
    // Mudar valor do item selecionado
    if(digitalRead(BTN_DOWN) == LOW) {
      if(menu_index == 0) { // Fonte
        current_source = (ControlSource)((current_source + 1) % 3);
      }
